(function(){"use strict";const select=(el,all=!1)=>{el=el.trim()
  ...
  window.addEventListener('load',()=>{let menuContainer=select('.menu-container');if(menuContainer){let menuIsotope=new Isotope(menuContainer,{itemSelector:'.menu-item',layoutMode:'fitRows'});let menuFilters=select('#menu-flters li',!0);on('click','#menu-flters li',function(e){e.preventDefault();menuFilters.forEach(function(el){el.classList.remove('filter-active')});this.classList.add('filter-active');menuIsotope.arrange({filter:this.getAttribute('data-filter')});menuIsotope.on('arrangeComplete',function(){AOS.refresh()})},!0)}});const glightbox=GLightbox({selector:'.glightbox'});new Swiper('.events-slider',{speed:600,loop:!0,autoplay:{delay:5000,disableOnInteraction:!1},slidesPerView:'auto',pagination:{el:'.swiper-pagination',type:'bullets',clickable:!0}});new Swiper('.testimonials-slider',{speed:600,loop:!0,autoplay:{delay:5000,disableOnInteraction:!1},slidesPerView:'auto',pagination:{el:'.swiper-pagination',type:'bullets',clickable:!0},breakpoints:{320:{slidesPerView:1,spaceBetween:20},1200:{slidesPerView:3,spaceBetween:20}}});const galleryLightbox=GLightbox({selector:'.gallery-lightbox'});window.addEventListener('load',()=>{AOS.init({duration:1000,easing:'ease-in-out',once:!0,mirror:!1})});$(window).scroll(function(){if($(this).scrollTop()>100){$('#whatsappBtn').fadeIn()}else{$('#whatsappBtn').fadeOut()}})})()